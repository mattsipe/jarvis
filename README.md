...
JARVIS can run quietly in the background and wake on the local keyword "Jarvis" — see `src/main/presence/` and Command Center's **Presence** panel. Wake-word detection runs fully on-device using [openWakeWord](https://github.com/dscripka/openWakeWord)'s official "hey jarvis" model via ONNX Runtime (see `resources/wakeword/NOTICE.md`) — no account, API key, or Internet access required, during or after installation, and Deepgram/Claude/ElevenLabs are never touched until the wake word actually fires. Presence just works out of the box; the Control+Space hotkey remains as a fallback either way. Global hotkeys: `Control+Space` toggles a conversation, `Control+Shift+Space` shows/hides Command Center, `Control+Shift+M` mutes/unmutes the mic (wake word included). Saying "that's all", "go back to sleep", or "end conversation" ends a session without a Claude call.
